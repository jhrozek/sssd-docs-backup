...
`​mod\_wsgi <http://code.google.com/p/modwsgi/>`__ is an Apache module
for running WSGI-compatible Python applications directly on top of the
Apache webserver. The mod\_wsgi adapter is written completely in C and
provides very good performances.

Overview
^^^^^^^^

#. `The ``trac.wsgi`` script <#Thetrac.wsgiscript>`__

   #. `A very basic script <#Averybasicscript>`__
   #. `A more elaborate script <#Amoreelaboratescript>`__
   #. `Recommended ``trac.wsgi`` script <#Recommendedtrac.wsgiscript>`__

#. `Mapping requests to the script <#Mappingrequeststothescript>`__
#. `Configuring Authentication <#ConfiguringAuthentication>`__

   #. `Using Basic Authentication <#UsingBasicAuthentication>`__
   #. `Using Digest Authentication <#UsingDigestAuthentication>`__
   #. `Using LDAP Authentication <#UsingLDAPAuthentication>`__
   #. `Using SSPI Authentication <#UsingSSPIAuthentication>`__
   #. `Using Apache authentication with the Account Manager plugin's
      Login
      form <#UsingApacheauthenticationwiththeAccountManagerpluginsLoginform>`__
   #. `Example: Apache/mod\_wsgi with Basic Authentication, Trac being
      at the root of a virtual
      host <#Example:Apachemod_wsgiwithBasicAuthenticationTracbeingattherootofavirtualhost>`__

#. `Troubleshooting <#Troubleshooting>`__

   #. `Use a recent version <#Usearecentversion>`__
   #. `Getting Trac to work nicely with SSPI and 'Require
      Group' <#GettingTractoworknicelywithSSPIandRequireGroup>`__
   #. `Trac with PostgreSQL <#TracwithPostgreSQL>`__
   #. `Other resources <#Otherresources>`__
...
The ``TRAC_ENV`` variable should naturally be the directory for your
Trac environment (if you have several Trac environments in a directory,
you can also use ``TRAC_ENV_PARENT_DIR`` instead), while the
...
environment) you must *not* use ``os.environ['TRAC_ENV']`` to set the
...
be filled with the path of a previously viewed Trac environment.
...
`TracInstall#cgi-bin <https://docs.pagure.org/sssd-test2/TracInstall.html#cgi-bin>`__).
...
If you followed the directions `​Generating the Trac cgi-bin
directory <http://trac.edgewall.org/wiki/TracInstall#cgi-bin>`__, your
Apache configuration file should look like following:
...
``WSGIApplicationGroup`` directive ensures that Trac is always run in
the first Python interpreter created by mod\_wsgi; this is necessary
...
the `​mod\_wsgi's wiki <http://code.google.com/p/modwsgi/wiki/>`__ and
...
See also `​Authentication, Authorization and Access
Control <http://httpd.apache.org/docs/2.2/howto/auth.html>`__ in the
Apache guide.
...
| See also the
  `​mod\_auth\_basic <http://httpd.apache.org/docs/2.2/mod/mod_auth_basic.html>`__
  documentation.
...
You'll have to create your ``.htpasswd`` file with the ``htdigest``
command instead of ``htpasswd``, as follows:
...
See also the
`​mod\_auth\_digest <http://httpd.apache.org/docs/2.2/mod/mod_auth_digest.html>`__
documentation.
...
Configuration for
`​mod\_ldap <http://httpd.apache.org/docs/2.2/mod/mod_ldap.html>`__
...
-  `​mod\_authnz\_ldap <http://httpd.apache.org/docs/2.2/mod/mod_authnz_ldap.html>`__,
   documentation for mod\_authnz\_ldap

-  `​mod\_ldap <http://httpd.apache.org/docs/2.2/mod/mod_ldap.html>`__,
   documentation for mod\_ldap, which provides connection pooling and a
   shared cache.
-  `​TracHacks:LdapPlugin <http://trac-hacks.org/wiki/LdapPlugin>`__ for
   storing
   `TracPermissions <https://docs.pagure.org/sssd-test2/TracPermissions.html>`__
   in LDAP.
...
`​mod-auth-sspi
project <http://sourceforge.net/projects/mod-auth-sspi/>`__ and then add
the following to your VirtualHost:
...
`​#1055 <http://trac.edgewall.org/intertrac/%231055>`__,
`​#1168 <http://trac.edgewall.org/intertrac/%231168>`__ and
`​#3338 <http://trac.edgewall.org/intertrac/%233338>`__.

See also
`​TracOnWindows/Advanced <http://trac.edgewall.org/intertrac/TracOnWindows/Advanced>`__.
...
plugin's `​Login
module <http://trac-hacks.org/wiki/AccountManagerPlugin/Modules#LoginModule>`__
and its `​HttpAuthStore authentication
module <http://trac-hacks.org/wiki/AccountManagerPlugin/AuthStores#HttpAuthStore>`__.
...
``trac-admin the-env initenv`` to create a folder ``the-env``, and you
used ``trac-admin the-env deploy the-deploy`` to create a folder
``the-deploy``, then first:
...
`​#100 <http://code.google.com/p/modwsgi/issues/detail?id=100>`__ and
`​#132 <http://code.google.com/p/modwsgi/issues/detail?id=132>`__.
...
`​#10675 <http://trac.edgewall.org/intertrac/%2310675>`__ for details).
...
This is done by setting ``poolable = False`` in
``trac.db.postgres_backend`` on the ``PostgreSQLConnection`` class.
...
troubleshooting <https://docs.pagure.org/sssd-test2/TracModPython.html#Troubleshooting>`__
section, as most Apache-related issues are quite similar, plus
discussion of potential `​application
issues <http://code.google.com/p/modwsgi/wiki/ApplicationIssues>`__ when
using mod\_wsgi. The wsgi page also has a `​Integration With
Trac <http://code.google.com/p/modwsgi/wiki/IntegrationWithTrac>`__
document.
...
`ModPython <https://docs.pagure.org/sssd-test2/TracModPython.html>`__,
