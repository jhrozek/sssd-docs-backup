...
-   The IPA provider now supports logins with enterprise principals
    (also known as additional UPN suffixes). This functionality also
    enabled Active Directory users from trusted AD domains who use an
    additional UPN suffix to log in. Please note that this feature
    requires a recent IPA server.
...
-   A new `ad_enabled_domains` option was added. This option lets the
    administrator select domains that SSSD should attempt to reach in
    the AD forest SSSD is joined to. This option is useful for
    deployments where not all domains are reachable on the network
    level, yet the administrator needs to access some trusted domains
    and therefore disabling the subdomains provider completely is not
    desirable.
-   The `sssctl` tool has two new commands `active-server` and `servers`
    that allow the administrator to observe the server that SSSD is
    bound to and the servers that SSSD autodiscovered
...
-   Several patches fix regressions caused by splitting the database
    into two ldb files, such as when user attributes change without
    increasing the `modifyTimestamp` attribute value
...
[\#2789](/sssd/ticket/2789 "Warn if ad_server contains IP address"){.closed}
:   Warn if ad\_server contains IP address

[\#2828](/sssd/ticket/2828 "Add an option to disable checking for trusted domains in the subdomains ..."){.closed}
:   Add an option to disable checking for trusted domains in the
    subdomains provider

[\#2856](/sssd/ticket/2856 "[RFE] Allow users to authenticate with alternative names"){.closed}
:   \[RFE\] Allow users to authenticate with alternative names

[\#2860](/sssd/ticket/2860 "Add support for disabling netlink use"){.closed}
:   Add support for disabling netlink use

[\#2948](/sssd/ticket/2948 "Handle overriden name of members in the memberUid attribute"){.closed}
:   Handle overriden name of members in the memberUid attribute

[\#2958](/sssd/ticket/2958 "Support multiple principals for IPA users"){.closed}
:   Support multiple principals for IPA users

[\#2978](/sssd/ticket/2978 "pid file name decalration is duplicated"){.closed}
:   pid file name decalration is duplicated

[\#2987](/sssd/ticket/2987 "Improve information about krb5_keytab & ldap_krb5_keytab option in sssd ..."){.closed}
:   Improve information about krb5\_keytab & ldap\_krb5\_keytab option
    in sssd man pages

[\#3009](/sssd/ticket/3009 "sssd fails to mark a connection as bad on searches that time out"){.closed}
:   sssd fails to mark a connection as bad on searches that time out

[\#3018](/sssd/ticket/3018 "Detect of IPA server can handle enterprise principals"){.closed}
:   Detect of IPA server can handle enterprise principals

[\#3024](/sssd/ticket/3024 "sssd-common brings in nfs-utils"){.closed}
:   sssd-common brings in nfs-utils

[\#3064](/sssd/ticket/3064 "incorrect dataExpireTimestamp setting in the timestamps cache"){.closed}
:   incorrect dataExpireTimestamp setting in the timestamps cache

[\#3068](/sssd/ticket/3068 "fixes to the initial config schema implementation"){.closed}
:   fixes to the initial config schema implementation

[\#3069](/sssd/ticket/3069 "The sssctl tool should provide information about active and available ..."){.closed}
:   The sssctl tool should provide information about active and
    available servers

[\#3072](/sssd/ticket/3072 "task: Add a 1.14 upstream repo"){.closed}
:   task: Add a 1.14 upstream repo

[\#3077](/sssd/ticket/3077 "sssd does not work under non-root user"){.closed}
:   sssd does not work under non-root user

[\#3084](/sssd/ticket/3084 "DP: Don't pass empty string, but NULL to providers"){.closed}
:   DP: Don't pass empty string, but NULL to providers

[\#3086](/sssd/ticket/3086 "tools: sssctl config-check and sssctl cache ignore --help"){.closed}
:   tools: sssctl config-check and sssctl cache ignore --help

[\#3087](/sssd/ticket/3087 "tools: make sssctl command names consistent"){.closed}
:   tools: make sssctl command names consistent

[\#3088](/sssd/ticket/3088 "Review and update SSSD's wiki pages for 1.14.1 release"){.closed}
:   Review and update SSSD's wiki pages for 1.14.1 release

[\#3089](/sssd/ticket/3089 "Error message "Failed to retrieve users" is sometimes misleading"){.closed}
:   Error message "Failed to retrieve users" is sometimes misleading

[\#3090](/sssd/ticket/3090 "Don't print message about trust direction on clients"){.closed}
:   Don't print message about trust direction on clients

[\#3091](/sssd/ticket/3091 "remove DEBUG(SSSDBG_TRACE_INTERNAL, "Trace: ldap_result found ..."){.closed}
:   remove DEBUG(SSSDBG\_TRACE\_INTERNAL, "Trace: ldap\_result found
    nothing!\\n");

[\#3093](/sssd/ticket/3093 "Missing nested groups in user groups"){.closed}
:   Missing nested groups in user groups

[\#3094](/sssd/ticket/3094 "SELINUX_getpeercon failed [-1][Unknown error -1]."){.closed}
:   SELINUX\_getpeercon failed \[-1\]\[Unknown error -1\].

[\#3096](/sssd/ticket/3096 "sssctl: Time stamps without time zone information"){.closed}
:   sssctl: Time stamps without time zone information

[\#3101](/sssd/ticket/3101 "sssd does not start if sub-domain user is used with simple access provider"){.closed}
:   sssd does not start if sub-domain user is used with simple access
    provider

[\#3109](/sssd/ticket/3109 "Wrong pam error code returned for password change in offline mode"){.closed}
:   Wrong pam error code returned for password change in offline mode

[\#3110](/sssd/ticket/3110 "Access denied after activating user in 389ds"){.closed}
:   Access denied after activating user in 389ds

[\#3111](/sssd/ticket/3111 "sssd doesn't start on IPA client if IPA server VM is paused"){.closed}
:   sssd doesn't start on IPA client if IPA server VM is paused

[\#3120](/sssd/ticket/3120 "SSSD fails to start when ldap_user_extra_attrs contains mail"){.closed}
:   SSSD fails to start when ldap\_user\_extra\_attrs contains mail

[\#3121](/sssd/ticket/3121 "[abrt] [faf] sssd: unknown function(): /usr/libexec/sssd/sssd_nss killed ..."){.closed}
:   \[abrt\] \[faf\] sssd: unknown function():
    /usr/libexec/sssd/sssd\_nss killed by 11

[\#3122](/sssd/ticket/3122 "Do not check local users with disabled local_negative_timeout"){.closed}
:   Do not check local users with disabled local\_negative\_timeout

[\#3130](/sssd/ticket/3130 "Better error message if sssctl is ran w/o activating the IFP responder"){.closed}
:   Better error message if sssctl is ran w/o activating the IFP
    responder

[\#3132](/sssd/ticket/3132 "check return value of sysdb_search_user_by_upn()"){.closed}
...
-   RESPONDERS: Show a bit more info in case of SELINUX\_getpeercon()
    failure
-   RESPONDERS: Pass errno to strerror() when SELINUX\_getpeercon()
    fails
