...
#### Smart Card related enhancements {#SmartCardrelatedenhancements}
...
#### Enhancements for easier administration {#Enhancementsforeasieradministration}
...
#### Two-factor authentication improvements {#Two-factorauthenticationimprovements}
...
-   With a recent IPA server (4.4.x), it is now possible to authenticate
    ...
    enabled and hits enter during the password prompt, the
    authentication will proceed with single factor only. The
    authentication method is stored in the ticket and depending on
    configuration of Kerberized services, the user will be able to only
    access selected service unless he authenticates with the second
    factor as well.
...
#### Miscellanous improvements {#Miscellanousimprovements}
...
#### Developer-facing features {#Developer-facingfeatures}
...
#### Internal changes {#Internalchanges}
...
-   The default attribute mappings for autofs provider with
    `ldap_schema` set to `rfc2307` were changed. The `rfc2307` schema
    now uses the `nis*` attribute maps. This is especially important for
    users who store their automounter maps on an Active Directory
    server, which typically lacks the RFC2307bis extensions.
...
[\#2011](/sssd/ticket/2011 "sysdb API is not consistent about the RDN of subdomain users"){.closed}
:   sysdb API is not consistent about the RDN of subdomain users

[\#2269](/sssd/ticket/2269 "[RFE] SSSD configuration file test tool (sssd_check)"){.closed}
:   \[RFE\] SSSD configuration file test tool (sssd\_check)

[\#2449](/sssd/ticket/2449 "Enable the sssd krb5 localauth plugin by default"){.closed}
:   Enable the sssd krb5 localauth plugin by default

[\#2788](/sssd/ticket/2788 "Allow fallback to default krb5_kuserok() implementation for root user from ..."){.closed}
:   Allow fallback to default krb5\_kuserok() implementation for root
    user from localauth plugin

[\#2838](/sssd/ticket/2838 "full_name_format and default_domain_suffix breaks supplemental AD trust ..."){.closed}
:   full\_name\_format and default\_domain\_suffix breaks supplemental
    AD trust groups

[\#2858](/sssd/ticket/2858 "Please fix rfc2307 ldap schema implementation"){.closed}
:   Please fix rfc2307 ldap schema implementation

[\#2919](/sssd/ticket/2919 "Improve sudo protocol"){.closed}
:   Improve sudo protocol

[\#2929](/sssd/ticket/2929 "NSS responder does not lowercase AD external group members"){.closed}
:   NSS responder does not lowercase AD external group members

[\#2966](/sssd/ticket/2966 "Support authentication indicators from IPA"){.closed}
:   Support authentication indicators from IPA

[\#2988](/sssd/ticket/2988 "[RFE] Make OTP/2FA authentication optional"){.closed}
:   \[RFE\] Make OTP/2FA authentication optional

[\#3003](/sssd/ticket/3003 "IPA key authentication logs false error message for root user."){.closed}
:   IPA key authentication logs false error message for root user.

[\#3041](/sssd/ticket/3041 "SSSD IPA provider should request client principal canonicalization by ..."){.closed}
:   SSSD IPA provider should request client principal canonicalization
    by default

[\#3055](/sssd/ticket/3055 "The sssctl tool is missing a manpage"){.closed}
:   The sssctl tool is missing a manpage

[\#3059](/sssd/ticket/3059 "The sssctl tool doesn't work with users from subdomains"){.closed}
:   The sssctl tool doesn't work with users from subdomains

[\#3062](/sssd/ticket/3062 "amend the sssd.conf manpage to mention include directories"){.closed}
:   amend the sssd.conf manpage to mention include directories

[\#3066](/sssd/ticket/3066 "sssctl remove-cache should restart, not stop sssd"){.closed}
:   sssctl remove-cache should restart, not stop sssd

[\#3071](/sssd/ticket/3071 "Review and update SSSD's wiki pages for 1.14.0 release"){.closed}
:   Review and update SSSD's wiki pages for 1.14.0 release

[\#3079](/sssd/ticket/3079 "endianess issues in NSS tests and sysdb utils"){.closed}
:   endianess issues in NSS tests and sysdb utils

[\#3135](/sssd/ticket/3135 "[RFE] Functionality to change the default OCSP responder"){.closed}
:   \[RFE\] Functionality to change the default OCSP responder

[\#3190](/sssd/ticket/3190 "sssd debug logging prints "No matching domain found for [$id]" everytime ..."){.closed}
...
-   SSS\_CACHE: Don't use sss\_get\_domain\_name, but create the
    internal fqname instead for users and groups
...
-   LDAP: Lookup services by all protocols unless a protocol is
    specified
...
-   sysdb: Use ldb\_result as output in
    sysdb\_search\_ts\_{users,groups}
...
-   TESTS: Start fixing the PAM responder tests for fully qualified
    names in sysdb
