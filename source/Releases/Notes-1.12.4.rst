...
-  A new option ``ad_site`` was added. When this option is set, SSSD
   will attempt to connect to DCs from this particular AD site instead
   of looking up the site via DNS
-  The ``ad_gpo_map_permit`` option now also includes the
   ``systemd-user`` service to avoid errors in processing of the PAM
   session stack
...
-  SELINUX: Set and reset umask when caling set\_seuser from deamon code
