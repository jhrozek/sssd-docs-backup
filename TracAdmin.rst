...
`​WebAdmin <http://trac.edgewall.org/intertrac/WebAdmin>`__ plugin now
integrated within Trac (since version 0.11).
...
commands (``help`` and ``initenv``), the same list you'd get when
invoking ``trac-admin`` alone. Also, ``trac-admin --version`` will tell
you about the Trac version (e.g. 0.12) corresponding to the program.
...
It supports an extra ``--inherit`` option, which can be used to specify
...
Note that in version 0.11 of Trac, ``initenv`` lost an extra last
argument ``<templatepath>``, which was used in previous versions to
point to the ``templates`` folder. If you are using the one-liner
...
default in ``trac-admin``. Note that this may not match the list given
by ``trac-admin <yourenv> help``, as the commands pertaining to
