...
#. `Trac Macros <#TracMacros>`__

   #. `Using Macros <#UsingMacros>`__

      #. `Getting Detailed Help <#GettingDetailedHelp>`__
      #. `Example <#Example>`__

   #. `Available Macros <#AvailableMacros>`__
   #. `Macros from around the world <#Macrosfromaroundtheworld>`__
   #. `Developing Custom Macros <#DevelopingCustomMacros>`__

      #. `Macro without arguments <#Macrowithoutarguments>`__
      #. `Macro with arguments <#Macrowitharguments>`__
...
`below <https://fedorahosted.org/sssd#AvailableMacros>`__.
...
| .. code:: wiki                       | .. raw:: html                        |
|                                      |                                      |
|     [[RecentChanges?(Trac,3)]]       |    <div class="trac-macrolist">      |
|                                      |                                      |
|                                      | .. rubric:: ``[[RecentChanges]]``    |
|                                      |    :name: RecentChanges-macro        |
|                                      |                                      |
...
|                                      | listed.                              |
|                                      |                                      |
|                                      | The second parameter is a number for |
...
|                                      | `InterTrac </wiki/InterTrac>`__      |
|                                      | prefixes.                            |
|                                      | .. rubric:: ``[[InterWiki]]``        |
|                                      |    :name: interwiki                  |
|                                      |                                      |
|                                      | Provide a description list for the   |
|                                      | known                                |
|                                      | `InterWiki </wiki/InterWiki>`__      |
|                                      | prefixes.                            |
|                                      | .. rubric:: ``[[KnownMimeTypes]]``   |
|                                      |    :name: knownmimetypes             |
|                                      |                                      |
|                                      | List all known mime-types which can  |
|                                      | be used as                           |
|                                      | `WikiProcessors </wiki/WikiProcessor |
|                                      | s>`__.                               |
|                                      | Can be …                             |
|                                      |                                      |
|                                      | .. raw:: html                        |
...
-  ``module:id:file``, where module can be either **wiki** or
   **ticket**, to refer to the attachment named *file* of the specified
   wiki page or ticket.
...
-  digits and unit are interpreted as the size (ex. 120, 25%) for the
   image
-  ``right``, ``left``, ``center``, ``top``, ``bottom`` and ``middle``
   are interpreted as the alignment for the image (alternatively, the
   first three can be specified using ``align=...`` and the last three
   using ``valign=...``)
-  ``link=some TracLinks...`` replaces the link to the image source by
   the one specified using a
   `TracLinks <https://docs.pagure.org/sssd-test2/TracLinks.html>`__. If
   no value is specified, the link is simply removed.
...
-  ``key=value`` style are interpreted as HTML attributes or CSS style
   ...
   -  ``border``, ``margin``, and ``margin-``\ \* can only be a single
      number
...
<`​gotoh@taiyo.co.jp <mailto:gotoh@taiyo.co.jp>`__>*

.. rubric:: ``[[InterTrac]]``
   :name: InterTrac-macro
...
-  The first is a number or range that allows configuring the minimum
   ...
   The default is to include all heading levels.
...
.. rubric:: ``[[RepositoryIndex]]``
   :name: RepositoryIndex-macro
...
+--------------------+--------------------------------------------------------------------------------------------------------------------------------------+
| ``titleindex``     | Only display the page name and title of each page, similar to `TitleIndex <https://docs.pagure.org/sssd-test2/TitleIndex.html>`__.   |
+--------------------+--------------------------------------------------------------------------------------------------------------------------------------+
...
`TracQuery#QueryLanguage <https://docs.pagure.org/sssd-test2/TracQuery.html#QueryLanguage>`__.
...
-  **list** -- the default presentation is to list the ticket ID next to
   the summary, with each ticket on a separate line.
-  **compact** -- the tickets are presented as a comma-separated list of
   ticket IDs.
-  **count** -- only the count of matching tickets is displayed
-  **table** -- a view similar to the custom query view (but without the
   controls)
...
The ``groupdesc`` parameter indicates whether the natural display order
of the groups should be reversed (defaults to **false**).
...
given to the macro, it will be used to specify the ``format``. Also,
using "&" as a field separator still works (except for ``order``) but is
deprecated.

.. rubric:: ``[[TitleIndex]]``
   :name: TitleIndex-macro
...
-  ``format=hierarchy``: The list of pages will be structured according
   ...
   ``min=n`` argument, where higher ``n`` flatten the display hierarchy
...
.. rubric:: ``[[TracAdminHelp]]``
   :name: TracAdminHelp-macro
...
.. rubric:: ``[[TracGuideToc]]``
   :name: TracGuideToc-macro

Display a table of content for the Trac guide.

This macro shows a quick and dirty way to make a table-of-contents for
the
`Help/Guide? <https://docs.pagure.org/sssd-test2/Help/Guide.html>`__.
The table of contents will contain the Trac\* and
`WikiFormatting <https://docs.pagure.org/sssd-test2/WikiFormatting.html>`__
pages, and can't be customized. Search for
`TocMacro? <https://docs.pagure.org/sssd-test2/TocMacro.html>`__ for a a
more customizable table of contents.
...
`TracIni <https://docs.pagure.org/sssd-test2/TracIni.html>`__ page.
...
The `​Trac Hacks <http://trac-hacks.org/>`__ site provides a wide
...
Macros, like Trac itself, are written in the `​Python programming
language <http://python.org/>`__ and are developed as part of
`TracPlugins <https://docs.pagure.org/sssd-test2/TracPlugins.html>`__.

For more information about developing macros, see the `​development
resources <http://trac.edgewall.org/intertrac/TracDev>`__ on the main
project site.

Here are 2 simple examples showing how to create a Macro with Trac 0.11.

Also, have a look at
`​Timestamp.py <http://trac.edgewall.org/intertrac/source%3Atags/trac-0.11/sample-plugins/Timestamp.py>`__
for an example that shows the difference between old style and new style
macros and at the
`​macros/README <http://trac.edgewall.org/intertrac/source%3Atags/trac-0.11/wiki-macros/README>`__
...
`TracEnvironment <https://docs.pagure.org/sssd-test2/TracEnvironment.html>`__'s
``plugins/`` directory.
...
Note that ``expand_macro`` optionally takes a 4\ :sup:`th` parameter
*``args``*. When the macro is called as a
`WikiProcessor <https://docs.pagure.org/sssd-test2/WikiProcessors.html>`__,
it's also possible to pass ``key=value`` `processor
parameters <https://docs.pagure.org/sssd-test2/WikiProcessors.html#UsingProcessors>`__.
...
(using ``return Markup.escape(result)``) or, if this is indeed HTML,
wrap it in a Markup object (``return Markup(result)``) with ``Markup``
