        ...
        2.  [5.1.2. Responder-to-Backend
            ...
            2.  [5.1.2.2. pamHandler](#a5.1.2.2.pamHandler)
                ...
                2.  [5.1.2.2.2. Response (PAM Responder -&gt;
                    Backend)](#a5.1.2.2.2.ResponsePAMResponder-Backend)
    ...
    7.  [7.7. Backend](#a7.7.Backend)
        ...
        3.  [7.7.3. Responder-Facing
            ...
            2.  [7.7.3.2. Receiving SBus
                Messages](#a7.7.3.2.ReceivingSBusMessages)
...
1.  Monitor process initializes itself, which includes parsing
...
##### 3.2.2.2. Data Flow (PAM Responder) {#a3.2.2.2.DataFlowPAMResponder}
...
### 3.3.1. Processes and Shared Objects {#a3.3.1.ProcessesandSharedObjects}
...
#### 3.3.1.4. Backend (aka Data Provider) {#a3.3.1.4.BackendakaDataProvider}
...
### 3.3.3. UNIX signals {#a3.3.3.UNIXsignals}
...
-   **Fast Cache (memcache)**: a single set of memory-mapped cache
    ...
    information, then the SSS Client falls back to using the NSS
...
4.2. Multiple Domains and Trust Relationships {#a4.2.MultipleDomainsandTrustRelationships}
---------------------------------------------
...
### 4.2.2. Domain Stanza vs. Domain {#a4.2.2.DomainStanzavs.Domain}
...
#### 4.2.3.2. Global Catalog (GC) {#a4.2.3.2.GlobalCatalogGC}
...
#### 5.1.1.1. SBus Connections {#a5.1.1.1.SBusConnections}
...
#### 5.1.1.2. Creating SBus Clients and Servers {#a5.1.1.2.CreatingSBusClientsandServers}
...
#### 5.1.1.3. Sending and Receiving SBus Messages {#a5.1.1.3.SendingandReceivingSBusMessages}
...
### 5.1.2. Responder-to-Backend API {#a5.1.2.Responder-to-BackendAPI}
...
### 5.2.2. Establishing an LDAP Connection {#a5.2.2.EstablishinganLDAPConnection}
...
6. Common Data Structures {#a6.CommonDataStructures}
=========================
...
### 7.2.3. PAM Client {#a7.2.3.PAMClient}
...
### 7.3.1. resp\_ctx {#a7.3.1.resp_ctx}
...
### 7.3.3.Common Optimization Techniques {#a7.3.3.CommonOptimizationTechniques}
...
4.   handler writes SBus message on backend socket            // client_send: uses write syscall 
...
7.6. Optimizations Code Flow {#a7.6.OptimizationsCodeFlow}
----------------------------
...
-   if NSS Responder does not find a valid entry in sysdb cache, it adds
    ...
    memcache (fast cache).
-   the memcache is not being updated anywhere here (it only gets
    updated when the Backend sends an initgrCheck SBus message to the
    NSS Responder).

7.7. Backend {#a7.7.Backend}
------------

In this section, we describe the functionality of a Backend, which
represent a domain stanza (e.g.
\[domain/[foo.com]{style="color: #FF0000"}\]. Recall that a domain
...
(e.g. ad\_domain=[foo.com]{style="color: #FF0000"}). As such, for each
domain stanza in the configuration, the Monitor spawns a separate
Backend process, and each Backend process dynamically loads its
configured provider plugins. The role of a provider plugin is to provide
plugin-specific implementations of generic functions (bet\_ops) used to
handle request messages, to perform check\_online operations, and to
perform finalize operations. Additionally, a provider plugin retrieves
plugin-specific configuration (pvt\_bet\_data), which it passes to each
...
### 7.7.1. Backend Concepts {#a7.7.1.BackendConcepts}

#### 7.7.1.1. Services and Servers {#a7.7.1.1.ServicesandServers}
...
3.  if successful, goto step 5; else, mark server offline for all
    services
4.  if more servers on list, goto step 2; else, SSSD enters offline
    mode; DONE
5.  connect to service on resolved server
6.  if successful, DONE; else, mark server offline for that service
7.  if more servers on list, goto step 2; else, SSSD enters offline
    mode; DONE

#### 7.7.1.2. Name Resolution {#a7.7.1.2.NameResolution}
...
-   if server specified with the “\_srv\_” string, we resolve the
    service using DNS SRV records
...
addressing information in server\_common-&gt;rhostent. If unsuccessful,
the recv function indicates that there was an error, in which case
upstream callers typically mark the server as being offline
(be\_mark\_offline).

#### 7.7.1.3. Configuration Lines {#a7.7.1.3.ConfigurationLines}
...
-   check\_online: determines whether SSSD is in “online mode” (i.e.
    ...
    pointer is only used by bet\_info\[BET\_ID\]. This function is
    called by check\_if\_online, which uses the results of check\_online
    to determine whether the Backend has transitioned to “online mode”,
    in which case the Backend runs any registered online callbacks. The
...
-   handler: provides the guts of the provider type's functionality; for
    example, the handler for bet\_info\[BET\_ID\] would implement
    identity lookups (e.g. contact the remote server, retrieve data,
    etc), while the handler for bet\_info\[BET\_AUTH\] would implement
...
-   loading and initializing provider plugins (aka Backend Modules), as
    ...
    -   this includes initializing the array of bet\_info structures
        with plugin-specific values

### 7.7.3. Responder-Facing Interactions {#a7.7.3.Responder-FacingInteractions}

#### 7.7.3.1. Registering Responders {#a7.7.3.1.RegisteringResponders}
...
#### 7.7.3.2. Receiving SBus Messages {#a7.7.3.2.ReceivingSBusMessages}
...
from bet\_info\[BET\_ID\]-&gt;bet\_ops-&gt;handler (which was registered
by be\_process\_init). We would also extract the argument with which to
...
7.8. AD Provider Plugin {#a7.8.ADProviderPlugin}
-----------------------
...
### 7.8.1. AD Provider Plugin: id\_provider {#a7.8.1.ADProviderPlugin:id_provider}
...
#### 7.8.1.1. ad\_id\_ctx {#a7.8.1.1.ad_id_ctx}
...
#### 7.8.1.2. ad\_account\_info\_handler {#a7.8.1.2.ad_account_info_handler}

This function is used to fulfill an identity lookup request. In this
section, we will use
getpwnam(“[aduser@foo.com]{style="color: #FF0000"}”) as our example. It
is called by be\_get\_account\_info, which is the generic sbus\_method
...
#### 7.8.1.3. ad\_check\_online {#a7.8.1.3.ad_check_online}
...
### 7.8.2. AD Provider Plugin: auth\_provider and chpass\_provider {#a7.8.2.ADProviderPlugin:auth_providerandchpass_provider}
...
#### 7.8.2.1. krb5\_auth\_ctx {#a7.8.2.1.krb5_auth_ctx}
...
#### 7.8.2.2. krb5\_pam\_handler {#a7.8.2.2.krb5_pam_handler}
...
-   obtains addressing information for the KDC (which is also the
    kpasswd server in AD)
...
##### 7.8.2.2.1. Parent =&gt; Child {#a7.8.2.2.1.ParentChild}
...
##### 7.8.2.2.2. Child=&gt;Parent {#a7.8.2.2.2.ChildParent}
...
#### 7.8.2.3. kdcinfo files {#a7.8.2.3.kdcinfofiles}
...
### 7.8.3. AD Provider Plugin: access\_provider {#a7.8.3.ADProviderPlugin:access_provider}
...
#### 7.8.3.1. ad\_access\_ctx {#a7.8.3.1.ad_access_ctx}
...
#### 7.8.3.2. ad\_access\_handler {#a7.8.3.2.ad_access_handler}

This function is used to fulfill an access check request, such as
determining whether the password of
"[aduser@foo.com]{style="color: #FF0000"}" has expired. It is called for
account management requests by be\_pam\_handler, which is the generic
sbus\_method function that handles *pamHandler* messages, the details of
...
-   the implementation of the comp computation:
    ...
    -   specifies private details for internal consumption by
        implementation
        -   ***comp****\_state: object used to pass around
            inputs/outputs between internal functions*
...
**read\_bytes\_send** and **read\_bytes***\_recv* functions, as well as
its own internal functions (such as ***read\_bytes***\_*handler*). The
caller calls the public interface with inputs/outputs, and also
...
main loop calls event-&gt;handler(..., event-&gt;handler\_data), just as
we would expect. In other words, the handler and handler\_data that we
...
8.3. Subrequests {#a8.3.Subrequests}
----------------
...
9.1. SDAP Connection Functions {#a9.1.SDAPConnectionFunctions}
------------------------------
...
### 9.1.1. sdap\_id\_op\_connect\_send/recv {#a9.1.1.sdap_id_op_connect_sendrecv}
...
### 9.1.2. sdap\_cli\_connect\_send/recv {#a9.1.2.sdap_cli_connect_sendrecv}
...
-   calls sdap\_connect\_send to establish a connection to the resolved
    server
-   calls sdap\_cli\_rootdse\_step to read rootDSE info from the
    resolved server (if anonymous access allowed)
-   calls cli\_kinit\_step to obtain addressing information for a KDC
    and to obtain a TGT from it
...
### 9.1.3. be\_resolve\_server\_send/recv {#a9.1.3.be_resolve_server_sendrecv}
...
-   if able to resolve an fo\_server in fo\_service-&gt;server\_list
    ...
    -   calls any cbs associated with service; for AD provider, this is
        ...
        -   sets service-&gt;sdap-&gt;uri=ldap: *srv-name; populates
            sockaddr with ip and LDAP\_PORT*
        -   sets service-&gt;gc-&gt;uri=ldap: *srv-name:3268; populates
            sockaddr with ip and GC\_PORT*
...
### 9.1.4. fo\_resolve\_service\_send/recv {#a9.1.4.fo_resolve_service_sendrecv}
...
-   if unable to resolve hostname for fo\_server
    ...
    -   set request error to EAGAIN, indicating that the caller should
        try the next fo\_server (if any)
...
fo\_server-&gt;server\_common-&gt;rhostent. If unsuccessful, return
EAGAIN, indicating that caller should try next fo\_server (if any).
...
### 9.1.5. resolv\_gethostbyname\_send/recv {#a9.1.5.resolv_gethostbyname_sendrecv}
...
### 9.1.6. resolv\_gethostbyname\_dns\_send/recv {#a9.1.6.resolv_gethostbyname_dns_sendrecv}
...
### 9.1.7. sdap\_connect\_send/recv {#a9.1.7.sdap_connect_sendrecv}
...
-   if connection successfully established,
    ...
    -   if not using start\_tls, set request done; sdap\_handle output
        can be retreived from state-&gt;sh
    -   if using start\_tls, calls ldap\_start\_tls,
        sdap\_set\_connected, sdap\_op\_add
...
### 9.1.8. sss\_ldap\_init\_send/recv {#a9.1.8.sss_ldap_init_sendrecv}
...
### 9.1.9. sdap\_get\_rootdse\_send/recv {#a9.1.9.sdap_get_rootdse_sendrecv}
...
### 9.1.10. sdap\_kinit\_send/recv {#a9.1.10.sdap_kinit_sendrecv}
...
### 9.1.11. sdap\_get\_tgt\_send/recv {#a9.1.11.sdap_get_tgt_sendrecv}
...
### 9.1.12. sdap\_auth\_send/recv {#a9.1.12.sdap_auth_sendrecv}
...
9.2. SDAP Operation Request Functions {#a9.2.SDAPOperationRequestFunctions}
-------------------------------------

### 9.2.1. users\_get\_send/recv {#a9.2.1.users_get_sendrecv}
...
### 9.2.2. sdap\_get\_generic\_send/recv {#a9.2.2.sdap_get_generic_sendrecv}
...
### 9.2.3. sdap\_get\_generic\_ext\_send/recv {#a9.2.3.sdap_get_generic_ext_sendrecv}
...
10. Filesystem Locations {#a10.FilesystemLocations}
========================
...
installation artifacts for each component.
  ...
  *Component*          *Source File Location*           *Installation Location*                                        *Log File and Prefix*
  ...
  Backend              providers/data\_provider\_be.c   /usr/libexec/sssd/sssd\_be                                     sssd\_[foo.com]{style="color: #FF0000"}.log;
  ...
  System DB            db/sysdb.c                       /var/lib/sss/db/cache\_[foo.com]{style="color: #FF0000"}.ldb   n/a
  -------------------- -------------------------------- -------------------------------------------------------------- ----------------------------------------------

11. Helpful Links {#a11.HelpfulLinks}
=================

-   SSSD Overview Video
    -   Presented by Stephen Gallagher-
        [[​]{.icon}http://www.youtube.com/user/opensourceidm](http://www.youtube.com/user/opensourceidm){.ext-link}
-   SSSD Wiki -
    [[​]{.icon}https://fedorahosted.org/sssd/wiki/](https://fedorahosted.org/sssd/wiki/){.ext-link}
    -   This is the main repository for SSSD information. It includes:
        -   \[Design Docs\] -
            [[​]{.icon}https://fedorahosted.org/sssd/wiki/DesignDocs](https://docs.pagure.org/sssd-test2/DesignDocs.html){.ext-link}
        -   \[Developer Pages\] -
            [[​]{.icon}https://fedorahosted.org/sssd/wiki/DevRes](https://docs.pagure.org/sssd-test2/DevRes.html){.ext-link}
            -   such as Developer Tips and Tutorials
        -   \[Documentation\] -
            [[​]{.icon}https://fedorahosted.org/sssd/wiki/Documentation](https://docs.pagure.org/sssd-test2/Documentation.html){.ext-link}
            -   such as Man Pages, Deployment Guides, Conference
                Presentations
-   Realmd Page -
    [[​]{.icon}http://www.freedesktop.org/software/realmd/](http://www.freedesktop.org/software/realmd/){.ext-link}
    -   \[Realmd Page\] This is the main repository for Realmd
        information. Among other things, it includes man pages, as well
        as an Administrative Guide.
-   Samba Components
    -   \[Talloc Tutorial\] -
        [[​]{.icon}http://talloc.samba.org/talloc/doc/html/libtalloc\_\_tutorial.html](http://talloc.samba.org/talloc/doc/html/libtalloc__tutorial.html){.ext-link}
    -   \[Tevent Tutorial\] -
        [[​]{.icon}http://tevent.samba.org/tevent\_tutorial.html](http://tevent.samba.org/tevent_tutorial.html){.ext-link}
    -   \[LDB Tutorial\] -
        [[​]{.icon}http://wiki.samba.org/index.php/Samba4/LDBIntro](http://wiki.samba.org/index.php/Samba4/LDBIntro){.ext-link}
-   FreeIPA Design Proposals (out-of-date)
    -   SSSD was originally conceived of as the IPAv2 Client, so these
        ...
        Service Controller Daemon in these documents refers to what
        later became known as the SSSD Monitor process.
        -   \[Design Overview\]
            [[​]{.icon}http://www.freeipa.org/page/V2/IPA\_Client\_Design\_Overview](http://www.freeipa.org/page/V2/IPA_Client_Design_Overview){.ext-link}
        -   \[Monitor\]
            [[​]{.icon}http://www.freeipa.org/page/SSSD/Service\_Controller\_Daemon](http://www.freeipa.org/page/SSSD/Service_Controller_Daemon){.ext-link}
        -   \[SBus\]
            [[​]{.icon}http://www.freeipa.org/page/SSSD/SBUS](http://www.freeipa.org/page/SSSD/SBUS){.ext-link}
