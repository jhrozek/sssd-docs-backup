...
-  the ``sss_cache`` utility gained a new option ``-E``. This option is
   a shortcut to tell ``sss_cache`` to invalidate all entries in the
   cache. Please note that invalidating sudo rules is still not
   implemented as it requires cooperation with the back end as well.
...
-  Every time release allocated memory in function py\_sss\_getgrouplist
