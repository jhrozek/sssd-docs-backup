...
`TracQuery#NavigatingTickets <https://docs.pagure.org/sssd-test2/TracQuery.html#NavigatingTickets>`__).
*(since 0.11)*
...
`TracRss <https://docs.pagure.org/sssd-test2/TracRss.html>`__ for
...
-  **realm** — together with **id**, can be used to create links to
   other resources than tickets (e.g. a realm of *wiki* and an *id* to a
   page name will create a link to that wiki page)
...
`below <https://fedorahosted.org/sssd#column-syntax>`__.

See
`​trac:wiki/CookBook/Configuration/Reports <http://trac.edgewall.org/intertrac/wiki/CookBook/Configuration/Reports>`__
...
-  **``__group__``** — Group results based on values in this column.
   Each group will have its own header and table.
...
-  **``_column``** — *Hide data*. Prepending an underscore ('\_') to a
   ...
   resource, e.g. ``id as _id`` will hide the **Id** column but the link
   to the ticket will be present.
...
clauses. See
`​TracIniReportCustomFieldSample <http://trac.edgewall.org/intertrac/TracIniReportCustomFieldSample>`__
for some examples.
...
`​Query Language Understood by
SQLite <http://www.sqlite.org/lang_expr.html>`__
